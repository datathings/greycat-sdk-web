...
import { Canvas, RectOptions, SimpleTooltip } from '../../chart-utils';
import { Disposable } from '../../internals';
import { getCSSVars, getHeatmapColors } from '../../utils';
 ...
 */
export class GuiHeatmapChart extends HTMLElement implements HeatmapChartProps {
  private _svg:
  ...
  connectedCallback() {
    ...
    const [
      ...
      nullValueColor = this._nullValueColor,
    ] = getCSSVars(this, 'heatmap-selected-color', 'heatmap-null-color');
  ...
  private _initialize() {
    ...
    const onHover = () => {
      ...
      const yIndex = this._scaleBandInvert(this._yAxis)(
        canvas.height - this._cursor.y
      );
        ...
        if (this._tooltip) {
          this._tooltip.updateRows([
            ...
            },
            {
              ...
              value: {
                value: `${table.data?.[yIndex[1]]?.[xIndex[1]] ?? ''}`,
                raw: true,
              },
            },
          ]);
    ...
    const canvas = (this._canvas = new Canvas(
      width -
        this._m.left -
        this._m.right -
        this._colorScaleWidth -
        this._colorScaleM.right,
    ...
    const colorScaleLeft =
      this._canvas.width + canvasLeft + this._colorScaleM.left;
  ...
  render() {
    ...
    if (yTicks !== undefined && yLabels) {
      const newYLabels = yLabels.filter(
        (_v, i) => !(i % Math.floor(yLabels.length / yTicks))
      );
      yAxis.tickValues(newYLabels);
    }
    if (xTicks !== undefined && xLabels) {
      const newXLabels = xLabels.filter(
        (_v, i) => !(i % Math.floor(xLabels.length / xTicks))
      );
    ...
    table.data.forEach((_v, row) => {
      _v.forEach((val, col) => {
        ...
        if (color) {
          const px = this._xAxis(this.xLabels?.[col] ?? `${col}`);
          const py = this._yAxis(this.yLabels?.[row] ?? `${row}`);
          const opts: RectOptions = {
            color: color,
            width: rectWidth,
            height: rectHeight,
    ...
    svg
      ...
      .attr(
        'transform',
        `translate(${this._m.left}, ${this._height - this._m.bottom})`
      )
      .call(xAxis)
      .selectAll('text')
      .attr('dx', '-2.2em')
    ...
    svg
      ...
      .attr(
        'transform',
        `translate(${canvas.width + this._m.left + this._colorScaleM.left}, ${
          this._colorScaleM.top
        })`
      )
...
declare global {
  interface Window {
    GuiHeatmapChart: typeof GuiHeatmapChart;
  }

  interface HTMLElementTagNameMap {
    'gui-heatmap-chart': GuiHeatmapChart;
  }

  namespace JSX {
    interface IntrinsicElements {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      'gui-heatmap-chart': any;
    }
  }
}

if (!window.customElements.get('gui-heatmap-chart')) {
  window.GuiHeatmapChart = GuiHeatmapChart;
  window.customElements.define('gui-heatmap-chart', GuiHeatmapChart);
}
