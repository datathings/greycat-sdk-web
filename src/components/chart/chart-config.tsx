...
export class GuiChartAxisInput extends HTMLElement {
  ...
  private _updateValue(
    input: sl.SlInput,
    value: number | bigint | core.time | Date | core.Date | undefined,
  ): void {
    if (value === undefined) {
      input.value = '';
   ...
   */
  private _getValue(input: sl.SlInput, scale: ScaleType): number | core.time | undefined {
    const value = input.value;
    if (value.length === 0) {
      return undefined;
    }
    if (scale === 'time') {
      return core.time.fromDate(new Date(value));
    }
    return +value;
  }
}
        ...
        <summary slot="summary">
          ...
          <sl-button
            ...
            onclick={(ev) => {
              ev.stopPropagation();
              ev.preventDefault();
              this.dispatchEvent(new XpChartConfigDeleteEvent(this.header, this));
            }}
          >
            Del
          </sl-button>
...
export class XpChartConfigDeleteEvent extends CustomEvent<{ name: string; el: Element }> {
  static readonly NAME = 'gui-chart-config-delete';

  constructor(name: string, el: Element) {
    super(XpChartConfigDeleteEvent.NAME, { detail: { name, el }, bubbles: true });
  }
}
...
declare global {
  ...
  interface GuiChartDeleteEventMap {
    [XpChartConfigDeleteEvent.NAME]: XpChartConfigDeleteEvent;
  }
