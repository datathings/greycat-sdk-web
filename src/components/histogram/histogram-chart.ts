import { core, Value } from '@greycat/sdk';
import * as d3 from 'd3';
import { Canvas } from '../../chart-utils/canvas.js';
import { SimpleTooltip } from '../../chart-utils/tooltip.js';
import { Bar, Line, Point, Rect } from '../../chart-utils/model.js';
import { getGlobalNumberFormat } from '../../globals.js';
import { emptyDataElement, getCSSVars, processCssVars } from '../../utils.js';
...
export class GuiHistogramChart extends HTMLElement {
  ...
  private _initialize() {
    ...
    const onHover = () => {
      ...
      if (table.cols.length > 0) {
        for (let row = 0; row < table.cols[0].length; row++) {
          const point: Value[] = [];
          for (let col = 0; col < table.cols.length; col++) {
            point.push(table.cols[col][row]);
          }
      ...
      const line: Line = table.cols.map((point, idx) => {
        const x = this._xAxis(Number(point[0]));
        const x2 = this._xAxis(Number(point[1]));
        zCumul += Number(point[this._columns[1]]);
        const z = this._zAxis(zCumul);
      ...
      if (nearestX > table.cols.length) {
        return requestAnimationFrame(onHover);
      }
      const data = table.cols[nearestX] as number[];
      this._cursorInfo = {
        x: line[nearestX].x,
        count: data[2],
        percentage: data[3],
  ...
  render() {
    ...
    const extent = d3.extent<number>([
      table.meta[0].min as number,
      table.meta[0].max as number,
      table.meta[1].min as number,
      table.meta[1].max as number,
    ]);

    canvas.ctx.clear();
    this._xAxis.domain([extent[0] ?? 0, extent[1] ?? 1]);
    const zSum = d3.sum(table.cols.map<number>((v) => Number(v[this._columns[1]])) ?? []);
    this._yAxis.domain([0, table.meta[this._columns[0]].max as number]);
...
declare global {
  ...
  namespace JSX {
    interface IntrinsicElements {
       ...
       */
      'gui-histogram-chart': Partial<Omit<GuiHistogramChart, 'children'>>;
    }
  }
}

if (!globalThis.customElements.get('gui-histogram-chart')) {
  globalThis.customElements.define('gui-histogram-chart', GuiHistogramChart);
}
