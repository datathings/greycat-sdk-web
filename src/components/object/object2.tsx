...
export class GuiObject2 extends GuiElement {
  ...
  update(): void {
    ...
    if (value instanceof GCObject) {
      ...
      if (tagName) {
        this.shadowRoot.replaceChildren(createElement(tagName, this._attrs));
        return;
      }
  ...
  private _renderObject(value: GCObject): void {
  ...
  private _renderEnum(value: GCEnum): void {
    this.shadowRoot.replaceChildren(<>{value.toString()}</>);
  }

  private _createObjField(objType: AbiType, attrName: string, _attrType: AbiType, value: unknown) {
