import { PrimitiveType, primitiveType, type IPrimitiveLoader, type Value } from './types.js';
import { GCObject, GCEnum } from './GCObject.js';
import { AbiPrecision, F64_DIVIDERS, Abi, type AbiType, type AbiFunction } from './abi.js';
...
export class AbiReader extends Reader implements Iterable<unknown> {
  ...
  *[Symbol.iterator](): Iterator<unknown> {
  ...
  read_object(): Value {
    ...
    return type.ctor.load(this, type);
  }
  ...
  read_array(len: number, can_skip = true): Array<Value> {
    ...
    for (let i = 0; i < len; i++) {
      ...
      if (all_slot_type === PrimitiveType.undefined) {
        ...
        arr[i] = abi_type.ctor.load(this, abi_type);
...
export class AbiWriter extends Writer {
  constructor(
    readonly abi: Abi,
    capacityOrBuffer: number | ArrayBuffer = 2048,
  ) {
  ...
  serialize(value: Value, target_type?: AbiType): void {
    ...
    } else if (target_type.generic_abi_type === this.abi.core.table && value instanceof gcreg.core.Table) {
      this.write_u8(PrimitiveType.object);
      this.write_vu32(target_type.offset);
  ...
  object(value: object): void {
    if (value instanceof GCObject) {
      ...
      new gcreg.core.Array(value).save(this);
    } else if (value instanceof Map) {
      new gcreg.core.Map(value).save(this);
    } else if (value === null) {
      this.null();
    } else {
  ...
  js_object(value: object): void {
    try {
      ...
      new gcreg.core.Map(new Map(Object.entries(value))).save(this);
    }
  }
  ...
  raw_js_object(value: object): void {
    try {
      ...
      // if we cannot find a type that matches, send the object as a Map
      new gcreg.core.Map(new Map(Object.entries(value))).saveContent(this);
    }
  }
