namespace gc {
  export namespace sdk {
    ...
    export class Abi {
       ...
       * @returns
       */
      create<T = GCObject>(typeFqn: string, fields?: Value[]): T {
      ...
      createValue<T = GCObject>(type: AbiType, fields?: Value[]): T {
        if (fields) {
    ...
    export class AbiType {
      ...
      constructor(
        ...
        if (is_enum) {
          ...
          if (module_name === 'core') {
            switch (type_name) {
              ...
              default: {
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                if ((gc.sdk.std_n.core as any)[type_name]) {
                  // eslint-disable-next-line @typescript-eslint/no-explicit-any
                  const GCObject = class extends (gc.sdk.std_n.core as any)[type_name] {
                    static readonly _type = type.name;
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
        ...
        if (!Object.hasOwn(gc, module_name)) {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          (gc as any)[module_name] = {};
        }
        // Store the constructor using its fqn
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (gc as any)[module_name][type_name] = this.ctor;
        // Store the constructor using the shortcut
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        if (typeof (gc as any)[type_name] === 'function') {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          delete (gc as any)[type_name];
        } else if (type_name.indexOf('$') !== -1) {
          // noop
        } else {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          (gc as any)[type_name] = this.ctor;
        }
      }
