...
export class duration extends GCPrimitive {
  ...
  static create(value: bigint | number, g: GreyCat = $.default): gc.core.duration {
    const ty = g.abi.types[g.abi.core.duration];
    return new ty.ctor(value) as duration;
  }

  static from_ms(value: number, g: GreyCat = $.default): gc.core.duration {
    const ty = g.abi.types[g.abi.core.duration];
    return new ty.ctor(value * Number(duration.MILLISECOND)) as gc.core.duration;
  }

  static from_secs(value: number, g: GreyCat = $.default): gc.core.duration {
    const ty = g.abi.types[g.abi.core.duration];
    return new ty.ctor(value * Number(duration.SECOND)) as gc.core.duration;
  }

  static from_mins(value: number, g: GreyCat = $.default): gc.core.duration {
    const ty = g.abi.types[g.abi.core.duration];
    return new ty.ctor(value * Number(duration.MINUTE)) as gc.core.duration;
  }

  static from_hours(value: number, g: GreyCat = $.default): gc.core.duration {
    const ty = g.abi.types[g.abi.core.duration];
    return new ty.ctor(value * Number(duration.HOUR)) as gc.core.duration;
  }

  static from_days(value: number, g: GreyCat = $.default): gc.core.duration {
    const ty = g.abi.types[g.abi.core.duration];
    return new ty.ctor(value * Number(duration.DAY)) as gc.core.duration;
  }

  static from_weeks(value: number, g: GreyCat = $.default): gc.core.duration {
    const ty = g.abi.types[g.abi.core.duration];
    return new ty.ctor(value * Number(duration.WEEK)) as gc.core.duration;
  }

  static from_months(value: number, g: GreyCat = $.default): gc.core.duration {
    const ty = g.abi.types[g.abi.core.duration];
    return new ty.ctor(value * Number(duration.MONTH)) as gc.core.duration;
  }

  static from_years(value: number, g: GreyCat = $.default): gc.core.duration {
    const ty = g.abi.types[g.abi.core.duration];
    return new ty.ctor(value * Number(duration.YEAR)) as gc.core.duration;
  }

  /**
   * Creates a `gc.core.duration` from a number `value` and a `gc.core.DurationUnit`.
   *
   * eg. `gc.core.duration.from_unit(42, gc.core.DurationUnit.seconds())` => `42s`
   */
  static from_unit(value: number, unit: gc.core.DurationUnit, g: GreyCat = $.default): gc.core.duration {
    switch (unit.key) {
      case 'microseconds':
