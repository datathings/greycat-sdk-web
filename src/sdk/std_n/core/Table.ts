...
export class Table<T = unknown[]> extends GCObject {
   ...
   */
  static create(cols: unknown[][] = [], g: GreyCat = $.default): gc.core.Table<unknown[]> {
   ...
   */
  static fromCols(cols: unknown[][] = [], g: GreyCat = $.default): gc.core.Table<unknown[]> {
  ...
  static fromMap<K, V>(map: globalThis.Map<K, V>, g: GreyCat = $.default): gc.core.Table<[K, V]> {
  ...
  /**
   * Returns either an object or an array depending on the underlying generic param of the gc.core.Table
   *
   ...
   */
  async inferMappings(g: GreyCat = $.default): Promise<gc.core.TableColumnMapping[]> {
    ...
    for (let i = 0; i < this.cols.length; i++) {
      ...
      row[i] = value;
      if (value instanceof gcreg.core.node) {
        hasNodes = true;
      }
    }
    let columns: unknown[];
    const nodes = row.map((v) => (v instanceof gcreg.core.node ? v : null)) as gc.core.node[];
    if (hasNodes) {
      columns = await gcreg.core.node.resolve_all(nodes, g);
    } else {
      columns = row;
    }
    ...
    for (let i = 0; i < columns.length; i++) {
      ...
      if (col instanceof gcreg.core.node) {
        // nested node
        const [value] = await gcreg.core.node.resolve_all([col], g);
        if (value instanceof GCObject && !value.$type.is_native) {
          for (let j = 0; j < value.$type.attrs.length; j++) {
            const attr = value.$type.attrs[j];
            mappings.push(new gcreg.core.TableColumnMapping(i, [attr.name]));
          }
        } else {
          mappings.push(new gcreg.core.TableColumnMapping(i, ['*']));
        }
