  ...
  export namespace std_n {
    export namespace core {
      ...
      export class Table<T = unknown[]> extends GCObject {
        ...
        static compare(a: any, b: any, ord: utils.SortOrd): number {
          let diff: number;
          ...
          return ord === utils.SortOrd.asc ? diff : -diff;
        }
        ...
        static create(
          cols: unknown[][] = [],
          g: GreyCat = $.default,
        ): greycat.core.Table<unknown[]> {
          const ty = g.abi.types[g.abi.core.table];
          return new ty.ctor(cols) as greycat.core.Table;
        }
         ...
         */
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        static fromRows(rows: any[][], g: GreyCat = $.default): greycat.core.Table<any[]> {
          const ty = g.abi.types[g.abi.core.table];
        ...
        static fromObjects<T extends object | null | undefined>(
          ...
          if (rows.length === 0) {
            const ty = g.abi.types[g.abi.core.table];
            return new ty.ctor([]) as greycat.core.Table<unknown[]>;
          }
          ...
          const ty = g.abi.types[g.abi.core.table];
          const table = new ty.ctor(cols) as greycat.core.Table<unknown[]>;
          table.headers = keys;
          return table;
        }
        ...
        static fromMap<K, V>(
          ...
          g: GreyCat = $.default,
        ): greycat.core.Table<[K, V]> {
          const keys = globalThis.Array.from(map.keys());
          const values = globalThis.Array.from(map.values());
          const ty = g.abi.types[g.abi.core.table];
          const table = new ty.ctor([keys, values]) as greycat.core.Table<[K, V]>;
          table.headers = ['Key', 'Value'];
          return table;
        }

        static load<T extends Value = unknown>(r: AbiReader, ty: AbiType): greycat.core.Table<T> {
          ...
          return new ty.ctor(cols) as greycat.core.Table<T>;
        }
        ...
        sort(col: number, ord: utils.SortOrd): void {
