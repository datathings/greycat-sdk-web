...
import {
  openidServerAuth,
  openidPkceAuth,
  type OpenidServerSpec,
  type OpenidPkceSpec,
  type OpenidServerResult,
  type HandleRedirectResult,
} from './openid.js';
/**
...
function toStrategy(auth: WithoutAbiOptions['auth']): AuthStrategy<unknown> | null {
  ...
  throw new Error(`init: unknown auth form ${JSON.stringify(auth)}`);
}
...
export class GreyCat extends Emitter<GreyCatEvents> {
  ...
  constructor(
    ...
    exports: GreyCatWasmExports | undefined,
    ...
    if (timezone === undefined) {
      this.timezone =
        gcreg.core.TimeZone[new Intl.DateTimeFormat().resolvedOptions().timeZone as gc.core.TimeZone.Field];
    } else {
      this.timezone = gcreg.core.TimeZone[timezone];
    }
  ...
  async pollTasks(): Promise<gc.runtime.Task[]> {
    ...
    const history = await gcreg.runtime.Task.history(0, this._max_tasks);
    this.registerLogger(logger);
    this.emit('tasks-history', history);

    const logger2 = this.unregisterLogger();
    const running = await gcreg.runtime.Task.running();
  ...
  async await<T = unknown>(task: TaskLike<T>, opts: TaskOptions = {}, signal?: AbortSignal): Promise<T> {
    ...
    if (res.ok) {
      ...
      if (value instanceof gcreg.core.Error) {
        throw value;
      }
      return value as T;
    }
  ...
  createGeo(lat: number, lng: number): gc.core.geo {
    return this.abi.createGeo(lat, lng);
  }

  createFunctionByFqn(fqn: string): gc.core.function_ {
    return this.abi.createFunctionByFqn(fqn);
  }

  createFunction(mod: string, type: string | undefined, name: string): gc.core.function_ {
    return this.abi.createFunction(mod, type, name);
  }

  createNode(value: bigint): gc.core.node {
    return this.abi.createNode(value);
  }

  createNodeList(value: bigint): gc.core.nodeList {
    return this.abi.createNodeList(value);
  }

  createNodeIndex(value: bigint): gc.core.nodeIndex {
    return this.abi.createNodeIndex(value);
  }

  createNodeGeo(value: bigint): gc.core.nodeGeo {
    return this.abi.createNodeGeo(value);
  }

  createNodeTime(value: bigint): gc.core.nodeTime {
    return this.abi.createNodeTime(value);
  }

  createTime(value: bigint | number): gc.core.time {
    return this.abi.createTime(typeof value === 'bigint' ? value : BigInt(value));
  }

  createDuration(value: bigint | number): gc.core.duration {
  ...
  root(signal?: AbortSignal): Promise<gc.project.Root> {
    return gcreg.runtime.Runtime.root(this, signal);
  }
  ...
  parseTime(isoDate: string, tz = this.timezone): gc.core.time {
    ...
    const str_buf = new Uint8Array(exports.memory.buffer, str_ptr, isoDate.length);
    new TextEncoder().encodeInto(isoDate, str_buf);

    const res = exports.gc_dtz_time__parse(str_ptr, str_buf.byteLength, tz.offset, res_ptr);
    ...
    return new gcreg.core.time(epoch_us);
  }
  ...
  printTime(time: gc.core.time, tz = this.timezone, format = '%Y-%m-%dT%H:%M:%S%.3f%z'): string {
    ...
    // Wasm uses its stack backwards, starting by default at 1 page (64KB)
    // and going down towards 0. So we use the bottom of the stack for our data passing
    const format_buf = new Uint8Array(exports.memory.buffer, 0, format.length + 1);
    ...
    const n = exports.gc_dtz_time__print(BigInt(time.value), tz.offset, 0, out_ptr, 128);
    return new TextDecoder().decode(new Uint8Array(exports.memory.buffer, out_ptr, n));
  }
