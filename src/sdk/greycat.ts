namespace greycat {
  ...
  export interface GreyCat {
     ...
     */
    await<T = unknown>(task: utils.TaskLike, pollEvery?: number, signal?: AbortSignal): Promise<T>;
    ...
    private constructor(
      ...
      for (const fn of this.abi.functions) {
        const theFn = (...args: unknown[]) => {
          ...
          const greycat = args[fn.params.length] as GreyCat | undefined ?? this;
  ...
  export class TaskCancelled extends Error {
    constructor(readonly task: utils.TaskLike) {
